 ...
 * @author USUARIO
 */
public class popupPresupuestosP extends javax.swing.JInternalFrame {
    ...
    private void pasajesTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_pasajesTableMouseClicked
        VentanaPresupuesto.jTFTP.setText(modeloTabla.getValueAt(pasajesTable.getSelectedRow(), 1).toString());
        VentanaPresupuesto.jTFIP.setText(modeloTabla.getValueAt(pasajesTable.getSelectedRow(), 2).toString());
        Pasaje_Data buscar = new Pasaje_Data();
        VentanaPresupuesto.pasajePaquete = buscar.buscarPasaje(Integer.parseInt(modeloTabla.getValueAt(pasajesTable.getSelectedRow(), 0).toString()));
        VentanaPresupuesto.jBA.setEnabled(true);
        this.dispose();
    ...
    private void crearTabla() {
        ...
        modeloTabla.addColumn("Tipo Transporte");
        modeloTabla.addColumn("Importe");
        pasajesTable.setModel(modeloTabla);

    }

    public void listarPasajes() {
        int idCO = Integer.parseInt(VentanaPresupuesto.idCO.getText());
        int idCD = Integer.parseInt(VentanaPresupuesto.idCD.getText());
        Pasaje_Data listar = new Pasaje_Data();
        List<Pasaje> list = new ArrayList();
        list = listar.listarPasajesCOCD(idCO, idCD);
        for (Pasaje pas : list) {
            modeloTabla.addRow(new Object[]{pas.getIdPasajero(), pas.getTipoTransporte(), pas.getImporte()});
        }
